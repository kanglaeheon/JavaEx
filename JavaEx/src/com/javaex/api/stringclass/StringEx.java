...
//		stringBasixEx();
	...
	private static void stringMethodsEx() {
		...
		idx = str.indexOf("Java", idx + 4);	//	시작 인덱스 뒤로 밀기 (22번 인덱스)
		System.out.println("3st index of Java : " + idx);
		//	더 이상 검색어 없으면 -1
		idx = str.lastIndexOf("Java");	//	역방향 검색
		System.out.println("역방향 검색: " + idx);
		
		//	문자열 추출
		System.out.println("substring 5 ~: " + str.substring(5));	//	5 ~ 끝까지
		...
		System.out.println(s2.replace(" ", "") + s3.replace(" ", ""));
		System.out.println(s2.trim() + s3.trim());	//	trim() -> 앞뒤 공백문자 제거
...
//		String[] split = str.split(" ");	//	공백 문자를 기준, 문자열 분절
	...
	private static void stringBasixEx() {
		String s1 = "Java";	//	리터럴
